...
W=${W:-1080}; H=${H:-1920}; CRF=${CRF:-15}; PRE=${PRE:-slow}; OUTD=${OUTD:-cut2}
...
# o pai inteiro na frente: 1,90 a 10,23 do take, a fala vai de 2,5 a 10,2
cl p1 $P/source/T05.mov 1.90 250 1.00 1.07 0.50 GC
# a partir daqui a musica manda. tudo em tempos e meios tempos de 20 quadros
cl c1 $P/source/T04.mov 1.00  40 1.02 1.12 0.50 GA
cl c2 $P/source/T03.mov 3.40  30 1.12 1.02 0.50 GA
cl c3 $P/source/T02.mov 0.20  20 1.02 1.11 0.50 GA
cl c4 $P/source/T03.mov 2.20  20 1.11 1.02 0.50 GA
cl c5 $P/source/T04.mov 1.85  10 1.10 1.20 0.50 GA
cl c6 $P/source/T02.mov 1.45  10 1.18 1.32 0.50 GA
cl r1 $P/source/T01.mov 0.30 100 1.62 1.26 0.92 GB
cl s1 $P/source/T01.mov 4.60  40 1.20 1.34 0.92 GB
cl s2 $P/source/T04.mov 1.20  20 1.06 1.16 0.50 GA
cl s3 $P/source/T01.mov 6.70  30 1.45 1.30 0.86 GB
cl s4 $P/source/T03.mov 0.30  20 1.04 1.12 0.50 GA
cl s5 $P/source/T01.mov 8.10  30 1.28 1.44 0.94 GB
...
$FF -y -hide_banner -loglevel error -loop 1 -framerate 30 -i $OUTD/endbg.png -f lavfi -i anullsrc=r=48000:cl=stereo \
 -filter_complex "[0:v]scale=w='trunc(${W}*(1.00+0.04*t/1.333)/2)*2':h='trunc(${H}*(1.00+0.04*t/1.333)/2)*2':eval=frame:flags=bicubic,crop=${W}:${H},${GB},setsar=1,format=yuv420p[v]" \
 -map "[v]" -map 1:a -frames:v 40 -r 30 -c:v libx264 -preset $PRE -crf $CRF -c:a pcm_s16le -t 1.333 $OUTD/e1.mov
for i in p1 c1 c2 c3 c4 c5 c6 r1 s1 s2 s3 s4 s5 e1; do echo "file '$PWD/$OUTD/$i.mov'"; done > $OUTD/list.txt
