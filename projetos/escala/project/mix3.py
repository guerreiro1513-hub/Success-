...
ramp(HIT / FPS, 21.967, 0.30, 0.30)            # gancho: chiado da carne por baixo
ramp(21.967, 22.633, 0.30, 0.55)               # antecipacao, sobe um pouco
ramp(22.633, 22.640, 0.55, 0.26)              # reveal: a trilha manda
ramp(22.640, TOT / FPS, 0.26, 0.26)
Amb = A * env[:, None]
k = int(0.02 * SR); Amb[dst0:dst0 + k] *= np.linspace(0, 1, k)[:, None]

# ---- impacto sutil no reveal: sub de 48 Hz decaindo, sem clique ----
R = int(679 / FPS * SR); d = int(0.55 * SR); t = np.arange(d) / SR
