...
for V in com sem; do
  case $V in com) O=$E/guerreiros-BRONCA-FAKE-20s.mp4;; sem) O=$E/guerreiros-BRONCA-FAKE-20s-SEM-MUSICA.mp4;; esac
  $FF -y -hide_banner -loglevel error -i $WK/cut/base.mov -framerate 30 -i $WK/tx/t_%04d.png -i $WK/mix/final_$V.wav \
    -filter_complex "[0:v][1:v]overlay=0:0:format=auto,format=yuv420p[v]" -map "[v]" -map 2:a -frames:v 597 \
